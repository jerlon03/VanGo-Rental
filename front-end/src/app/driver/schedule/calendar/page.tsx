    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                ...
                setBookings(data.filter(booking =>
    ...
    return (
        <div className='bg-white rounded-lg shadow-lg p-6 max-w-6xl mx-auto'>
            ...
            <div className='border-b pb-4 flex justify-between w-full items-center'>
                <div>
                    <h1 className='text-[18px] font-semibold text-gray-800'>My Calendar</h1>
                ...
                <div className='flex gap-4 '>
                    <div>
                        ...
                        <span className='text-[10px] tracking-[1px] text-green-800 font-semibold'>ongoing</span>
                    </div>
                    <div>
                        <div className='w-full border h-[10] rounded-[5px] bg-primaryColor'></div>
                        <span className='text-[10px] tracking-[1px] text-primaryColor font-semibold'>upcoming</span>
                    </div>
                </div>
            </div>
            ...
            <div className='mt-6'>
                ...
                {/* Calendar Days */}
                <div className='grid grid-cols-7 gap-2'>
                    {Array.from({ length: getFirstDayOfMonth(displayDate) }).map((_, index) => (
                        <div key={`empty-${index}`} className='aspect-square'></div>
                    ))}

                    {Array.from({ length: getDaysInMonth(displayDate) }).map((_, index) => {
                        const date = new Date(displayDate.getFullYear(), displayDate.getMonth(), index + 1);
                        ...
                        return (
                            <div
                                key={index}
                                className={`aspect-square p-1 border rounded-lg ${isToday ? 'bg-blue-50 border-blue-300' : 'border-gray-200'
                                    }`}
                            >
                                <div className='h-full'>
                                    <div className={`text-right mb-1 text-[14px] ${isToday ? 'text-primaryColor font-semibold ' : 'text-blackColor font-semibold'
                                        }`}>
                                        {index + 1}
                                    </div>
                                    <div className='space-y-1 overflow-y-auto max-h-[70px]'>
                                        {dayBookings.length > 0 ? (
                                            dayBookings.map((booking, bookingIndex) => (
                                                <div
                                                    key={bookingIndex}
                                                    className={`text-xs p-1 rounded ${booking.status === 'ongoing'
                                                            ? 'bg-green-800 text-white'
                                                            : 'bg-primaryColor text-white'
                                                        }`}
                                                >
                                                    <div className='truncate'>
                                                        {new Date(booking.pickup_date_time).toLocaleTimeString([], {
                                                            hour: '2-digit',
                                                            minute: '2-digit'
                                                        })}
                                                    </div>
