        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData.message === 'Email does not exist') {
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: errorData.message || 'An error occurred while sending the email.',
                        icon: 'error',
            ...
            Swal.fire({
                ...
                window.open('https://accounts.google.com/v3/signin/identifier?continue=https%3A%2F%2Fmail.google.com%2Fmail%2F&emr=1&ltmpl=default&ltmplcache=2&osid=1&passive=true&rm=false&scc=1&service=mail&ss=1&ifkv=AcMMx-fPoSa09y4KG-adLY-xsoE1sS7VDGgi7IP1SWTfl4tnqRZkw336VuOSUv8Iy-RoLi4-s2Fp&ddm=0&flowName=GlifWebSignIn&flowEntry=ServiceLogin', '_blank');
            });
        ...
        <div className="relative w-full h-screen bg-blackColor/50 flex justify-center items-center text-blackColor">
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} width="400px" height="auto" className='items-center '>
                <div className={`transform bg-white rounded-[5px] text-blackColor py-8 px-6 transition-transform duration-500 ease-in-out ${isAnimating ? 'scale-100 opacity-100' : 'scale-50 opacity-0'}`} onAnimationEnd={() => setIsAnimating(false)}>
                    ...
                    <p className='text-[15px] mb-4'>Enter the email address you used to create the account and we will email you instructions to reset your password.</p>
                    {warning && <p className="text-red-500 mb-2">{warning}</p>} {/* Display warning message */}
                    <form onSubmit={handleSubmit} className='flex flex-col gap-4 items-center'>
                        ...
                        <Button type="submit" name='Send Email' backgroundColor='alert' height='40px'></Button>
                        <p className='text-[15px]'>Remember Password? <Link href='/login' className='text-button underline text-[15px]'>Login</Link></p>
                    </form>
                </div>
            </Modal>
        </div>
