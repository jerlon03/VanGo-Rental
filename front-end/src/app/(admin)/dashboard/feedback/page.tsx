...
import { getAllPrivateFeedbacks, updateFeedbackStatus } from '@/lib/api/feedback.api'
...
const FeedbackPage = () => {
    ...
    const updateStatusToPublish = (feedbackId: number) => {
        setFeedbacks((prevFeedbacks) =>
            prevFeedbacks.map((feedback) =>
                feedback.feedback_id === feedbackId ? { ...feedback, status: 'PUBLISH' } : feedback
            )
        );
    };
    ...
    const handlePublishClick = (feedbackId: number) => {
        ...
        SweetAlert.showConfirm("Do you want to publish this feedback?")
            .then(async (result) => {
                if (result) {
                    try {
                        ...
                        if (response && response.success) {
                            ...
                            setFeedbacks((prevFeedbacks) =>
                                prevFeedbacks.map((feedback) =>
                                    feedback.feedback_id === feedbackId ? { ...feedback, status: 'UNPUBLISH' } : feedback
                                )
                            );
    ...
    return (
        <div className='w-full'>
            <div className='w-full pb-3'>
                <AdminHeader>
                    <h1 className='text-[14px] flex h-full items-end text-blackColor/70 tracking-[2px]'><Link href="/dashboard">Dashboard</Link> / Customer Feedback</h1>
                </AdminHeader>
            </div>
            <div className='w-full px-[2%]'>
                {error ? (
                    ...
                    <DataTable
                        value={feedbacks} // Use filtered posts here
                            ...
                            body={(rowData) => (
                                <div className='flex justify-center'>
                                    {rowData.status !== 'PUBLISH' ? (
                                        ...
                                        <MdPublish
                                            onClick={() => SweetAlert.showWarning('This feedback is already published.')}
